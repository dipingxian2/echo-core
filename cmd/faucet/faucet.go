...
var (
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Echo Chain and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Echo Chain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Echo Chain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Echo Chain protocol stack
	client *ethclient.Client   // Client connection to the Echo chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Echo Chain light client protocol
...
// close terminates the Echo Chain connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Echo Chain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
...
// the username, avatar URL and Echo Chain address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Echo Chain address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Echo Chain address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Echo Chain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Echo Chain address,
